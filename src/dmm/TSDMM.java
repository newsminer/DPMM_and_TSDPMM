     ...
     public  void initTopicsWithKnowledge()
     {
         ...
         for (Doc doc:documents)
         {
        	 ...
        	 else
        	 {
        		 ...
        		 int selected = chooseTopicWithKnowledge(doc,topics,oldTopics,currentDocuments);
        		 if(selected==topics.size())
        		 {
        			 Topic t = new Topic();
            		 topics.add(t);
...
//     }
     ...
     public int chooseTopicWithKnowledge(Doc doc,List<Topic> all,HashSet<Topic> oldTopics, double currentDocuments)
     {
         ...
         for (int i = 0; i < logP.length-1; i++)
         {
             logP[i] = all.get(i).logCrpWithKnowledge(currentDocuments,oldTopics) + posteriorLogPdfWithKnowledge(doc,i) ; 
         }
         
         logP[logP.length-1] =Math.log(Global.gamma)-Math.log(currentDocuments-1+ oldTopics.size()*Global.alpha + Global.gamma) +  posteriorLogPdfWithKnowledge(doc,logP.length-1);
...
//     }
     ...
     protected  void sampleTopicWithKnowledge(Doc doc)
     {
         ...
         int selected = chooseTopicWithKnowledge(doc,topics,oldTopics, numDocuments);
         //Add Xi back to the sampled Cluster
         if(selected==topics.size()) { //if new cluster
//        	 if(!justRemove)
//        	 {
...
//                		 
//                	 }
//                      pw += (topics.get(k).wordCount[w_id]  + Global.beta) / (temp+ Global.vocabSize *Global.beta);   
//                 }
//                 logDoc += Math.log(pw);
//             }
//             
...
//     }
     ...
     {
         KnowledgeDMM corpus = new KnowledgeDMM();
         
         corpus.ReadFromDirectory("./5-MAR-1987");
         System.out.println("Reading finished");
         long startTime = System.currentTimeMillis();
        
         corpus.StartProcessing("load", 100);  
         long stopTime = System.currentTimeMillis();
         long elapsedTime = stopTime - startTime;
         System.out.println("Completed in "+String.valueOf(elapsedTime/1000.0)+" sec");
    
       //
         corpus.WriteTopicToFile("./new_twords.txt"); 
